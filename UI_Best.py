from svg_mark2 import layout_to_svg
from svg_mark2 import load_layout_from_db   # 👈 THIS IS REQUIRED
...
import os
from plot_drawer import PlotDrawerUI  
def init_db():
...
#Testing update mark1 display svg----------------------------------------
...
def generate_and_render_svg(project_id: str):
...
def page_home():
    st.title("KB Residential Design ")
    ...
    if not projects:
        ...
        for project_id, project_name, created_at in projects:
            with st.container():
                ...
                with col3:
                    if st.button("View", key=f"view_{project_id}"):
...
def page_form():
    ...
    # # Plot Drawing Tool Integration
    # st.markdown("**Visualize & Draw Plot Layout**")
    # st.session_state["plot_drawer.points"]

    # ui = PlotDrawerUI()
    # plot_json = ui.run()
    # form_data["plot_shape"] = plot_json
    ...
    st.subheader("Climate & Outdoor Features")
    ...
    st.subheader("Interior Design (Optional)")
...
def page_view():
    ...
    if st.session_state.current_svg:
        ...
        st.info("No SVG generated yet.")

    # ------------------ Iteration Controls ------------------
    col1, col2 = st.columns(2)

    with col1:
        if st.button("🔄 Generate / Update Layout", use_container_width=True):
            generate_and_render_svg(st.session_state.editing_project_id)
            st.session_state.iteration += 1
            st.rerun()

    with col2:
        if st.button("♻️ Reset Iteration", use_container_width=True):
            st.session_state.iteration = 1
            st.session_state.current_svg = None
            st.rerun()
